...
def test_spherical_mean_resultant_length() -> None:
    ...
    # Our implementation: norms[norms == 0] = 1.0, unit_vectors = [0,0] / 1.0 = [0,0]. Mean = [0,0].
    # Norm = 0.
    assert compute_spherical_mean_resultant_length(vecs) == 0.0


def test_centroid_dispersion() -> None:
...
def test_semantic_entropy() -> None:
...
def test_calculate_window_utilization() -> None:
...
def test_extract_token_usage_path_1() -> None:
...
def test_extract_token_usage_path_4() -> None:
...
def test_extract_token_usage_no_metadata() -> None:
    response = MagicMock()
...
def test_query_metrics_to_langfuse_scores() -> None:
