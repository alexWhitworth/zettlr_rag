...
from evals.reliability_harness import (
    ReliabilityHarness,
    _reliability_verdict,
    get_response_embeddings,
    main,
)
...
@pytest.fixture
def test_log_path(tmp_path: Any) -> str:
    return str(tmp_path / "test_validation_log.jsonl")


def test_reliability_verdict() -> None:
    # Consistent
...
def test_run_test(
    mock_semantic_entropy: MagicMock,
    mock_resultant_length: MagicMock,
    mock_dispersion: MagicMock,
...
def test_print_summary(capsys: Any, test_log_path: str) -> None:
...
def test_get_response_embeddings(mock_setup: MagicMock, mock_settings: MagicMock) -> None:
...
def test_main(
    ...
    mock_harness_cls.return_value = mock_harness_instance

    main()

    mock_harness_cls.assert_called_once_with(instrumented=False, semantic_entropy=False)
